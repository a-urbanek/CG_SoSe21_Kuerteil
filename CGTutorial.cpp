...
/*GLOBALE VARIABLEN FÜR DIE ACHSENROTATION*/

float rotateCam[] = { 0.0f, 0.0f, 0.0f };
float rotateSeg1[] = { 0.0f, 0.0f, 0.0f };
float rotateSeg2[] = { 0.0f, 0.0f, 0.0f };
float rotateSeg3[] = { 0.0f, 0.0f, 0.0f };

float t = 1.0f;
float s = 0.0f;
bool vzeich = false;

void rotateObj(float val, int pos, float obj[])
{
	obj[pos] += val;
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// if-statement "action == GLFW_PRESS": Was passieren soll, wenn eine Taste gedrückt wird

	switch (key)
	{

	case GLFW_KEY_W:
		rotateObj(10.0f, 0, rotateCam);
		break;
	...
	case GLFW_KEY_KP_1:
		rotateObj(10.0f, 2, rotateSeg2);
		break;
	case GLFW_KEY_KP_2:
		rotateObj(10.0f, 2, rotateSeg3);
		break;
	...
	// Was passieren soll, wenn eine Taste losgelassen wird
...
void drawCS() 
{
	...
	Model = glm::scale(Save, glm::vec3(3.0f, 0.01f, 0.01f));
	sendMVP();
	drawCube();

	Model = glm::scale(Save, glm::vec3(0.01f, 3.0f, 0.01f));
	sendMVP();
	drawCube();

	Model = glm::scale(Save, glm::vec3(0.01f, 0.01f, 3.0f));
	sendMVP();
	drawCube();
...
void drawPlat1()
{
	...
	Model = glm::scale(Save, glm::vec3(3.0f, 0.3f, 3.0f));
	sendMVP();
	drawCube();
...
void drawSeg(float h) 
{
	...
	Model = glm::scale(Model, glm::vec3(h / 5.0f, h / 5.0f, h / 5.0f));
	sendMVP();
	drawSphere(50, 50);
	Model = Save;
}
...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		View = glm::lookAt(glm::vec3(0,0,-8), // die Kamera ist bei (0,0,-5), in Weltkoordinaten
		...
		Model = glm::rotate(Model, rotateCam[0], glm::vec3(1.0f, 0.0f, 0.0f));
		Model = glm::rotate(Model, rotateCam[1], glm::vec3(0.0f, 1.0f, 0.0f));
		Model = glm::rotate(Model, rotateCam[2], glm::vec3(0.0f, 0.0f, 1.0f));
		...
		Model = glm::translate(Model, glm::vec3(t, 5.0f, 0.0f));
		Model = glm::rotate(Model, 90.0f, glm::vec3(0.0f, 1.0f, 0.0f));
		drawPlat1();
		
		Model = glm::translate(Model, glm::vec3(0, -10.0f, 0.0f));
		drawPlat1();

		
		Model = Save;

		Model = glm::translate(Model, glm::vec3(0.0f, s, 0.0f));
		drawSeg(2.0f);


		glm::vec4 lightPos = Model * glm::vec4(0.0f, 0.9f, 1.0f, 1.0f);
